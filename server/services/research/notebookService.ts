...
export class ResearchNotebookService {
  ...
  public async getNotebookSources(securityId: string, asOfDate?: string): Promise<ResearchSourceItem[]> {
    ...
    for (const ev of allEvidence) {
      const sourceKey = `${ev.sourceType}:${ev.provider}:${ev.sourceReference?.accessionNumber || ev.documentType || 'primary'}`;
  ...
  public async executeResearch(params: {
    ...
    const { securityId, queryType, customQuery, asOfDate, portfolioContext } = params;
  ...
  private compareSnapshotsWithPrior(
    ...
    const newItems = currentEvidenceIds.filter(id => !priorSet.has(id));
